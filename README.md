...
## מסלולי לקוח (Lifecycle)
- **לקוח חדש** — תשאול מלא → קביעת **פגישת היכרות** → *עוצרים כאן*. תאריכי השהייה
  **לא** נקבעים בצ'אט אלא בפגישה עצמה. הבעלים מזין ב-`owner.html` את התאריכים שסוכמו
  בפגישה (כפתור "＋ שריין תאריכי שהייה" ליד הפגישה) → הם מופיעים ביומן והלקוח מקבל הודעה.
- **תזכורת אוטומטית** — אם עברו `REMIND_AFTER_HOURS` שעות מהפגישה ולא שוריינו תאריכים,
  ה-Worker (Cron) שולח ללקוח תזכורת פעם אחת ("עדיין לא שיריינת תאריכים…").
- **לקוח חוזר** — מזוהה לפי טלפון (בוואטסאפ) או שם (בדמו), **מדלג על התשאול**
  ועובר ישר לשריון תאריכים בצ'אט.
