...
const AI_SYSTEM =
  ...
  'סגנון: דבר/י כמו אדם אמיתי ולא כמו טופס. תמיד קודם התייחס/י למה שהלקוח כתב עכשיו, ורק אז המשך/י. אל תחזור/י על שאלה שכבר נענתה — הסק/י פרטים מהקונטקסט. אם הלקוח מציין שהשיחה נקטעה/קרסה או שכבר דיברתם — התנצל/י בחום והמשך/י בעדינות בלי להתחיל מחדש. ' +
...
async function callClaude(env, messages, system) {
  const r = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({ model: env.MODEL || 'claude-sonnet-5', max_tokens: 2048, thinking: { type: 'adaptive' }, output_config: { effort: 'low' }, system: system || AI_SYSTEM, tools: AI_TOOLS, messages })
  });
