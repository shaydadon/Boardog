...
(function (global) {
  ...
  function ScriptedBot(io, saved) {
    ...
    return {
      start() { io.bot({ text: `ברוכים הבאים ל${K.name}! איזה כיף שפנית אלינו 🐾` }); setTimeout(ask, 600); },
      input(text) {
  ...
  const SYSTEM =
    ...
    'לאחר שקיבלת את שם הפונה — קרא/י ל-lookup_customer עם owner_name כדי לבדוק אם זה לקוח קיים.\n' +
  ...
  function AiBot(io, saved) {
    ...
    function dynamicSystem() {
      ...
      const shownName = ((p.kennelName || '').trim()) || K.name;
      extra += `\n\nההודעה הראשונה שלך בשיחה חייבת לפתוח במדויק במשפט: "ברוכים הבאים ל${shownName}! איזה כיף שפנית אלינו 🐾", ומיד באותה הודעה לשאול בעדינות לשם הפונה — שאלה אחת בלבד. אל תחזור/י על ברכת הפתיחה בהמשך השיחה.`;
      if (p.address) extra += `\n\nכתובת הפנסיון: ${p.address}. אם הלקוח שואל היכן הפנסיון / איך להגיע — מסור/מסרי את הכתובת הזו.`;
