...
(function (global) {
  ...
  function ScriptedBot(io) {
    ...
    return {
      start() { io.bot({ text: `שלום! 👋 הגעתם ל*${K.name}*. אני העוזר/ת החכם/ה של ${K.ownerName}, ואשמח לעזור לקלוט את הכלב שלכם לפנסיון.` }); setTimeout(ask, 500); },
      input(text) { const s = INTAKE[step]; if (s) answers[s.key] = text; setTimeout(ask, 300); },
      pickSlot,
      mode: 'scripted'
    };
  }
  ...
  const SYSTEM =
    ...
    'אסוף/אספי את הפרטים הבאים: שם הבעלים, שם הכלב, גזע, גיל, גודל, האם מעוקר/מסורס, האם החיסונים בתוקף, ' +
    'טיפול נגד פרעושים/קרציות, בעיות בריאות/תרופות, התאמה לכלבים אחרים, עבר של תוקפנות/נשיכות, אוכל ולו"ז, ' +
    'ותאריכי השהייה הרצויים. לאחר שאספת את כל הפרטים, קרא/י ל-get_available_slots והצג/י ללקוח 3–5 מועדים לפגישת היכרות. ' +
    'כשהלקוח בוחר מועד — קרא/י ל-book_meeting. לבסוף קרא/י ל-save_summary עם כל הנתונים והמועד שנקבע, והודה/י ללקוח. ' +
    'אל תמציא/י מדיניות; אם נשאלת משהו שאינך יודע/ת, אמור/י שתעביר/י ל' + K.ownerName + '.';
